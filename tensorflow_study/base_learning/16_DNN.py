...
#构建全连接层
# with tf.name_scope("dnn"):
#     # tensorflow使用这个函数帮助我们使用合适的初始化w和b的策略，默认使用ReLU激活函数，activation_fn=None设置为不需要激活函数
...
with tf.name_scope("loss"):
    ...
    # 传进来的数据是one-hot编码，用softmax_cross_entropy_with_logits这个函数，
...
with tf.Session() as sess:
    ...
    save_path = saver.save(sess, "./my_dnn_model/my_dnn_model_final.ckpt")
'''
# 使用模型预测
with tf.Session as sess:
